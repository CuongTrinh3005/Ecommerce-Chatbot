...
try:
    ...
    input = Input(shape=len(train_x[0]), dtype='int32', name='input')
    x = Dense(128, activation='relu')(input)
    x = Dropout(0.5)(x)
    x = Dense(128, activation='relu')(x)
    x = Dropout(0.5)(x)
    output = Dense(len(train_y[0]), activation='softmax')(x)
    ...
    # Fit model
    model.fit(np.asarray(train_x), np.asarray(train_y), epochs=1024, verbose=1)
    print(len(train_x[0]))
    print(len(train_y[0]))
# Save model
